...
matrix = []
miner_pos = [] # [miner_row, miner_col] #pos e position
...
for row in range(n):
    ...
    if 's' in matrix[row]:
        miner_pos = [row, matrix[row].index('s')] #indexa ni dava index na kolona
        matrix[miner_pos[0]][miner_pos[1]] = '*' #mahame miner-a zashtoto ne ni trqbva sled kato veche sme namerili poziciqta mu
